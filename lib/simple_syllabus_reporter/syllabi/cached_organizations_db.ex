defmodule SimpleSyllabusReporter.Syllabi.CachedOrganizationsDb do
  require Logger
  ...
  def upsert_organizations(orgs) when is_list(orgs) do
    if Enum.empty?(orgs) do
      ...
      sql = """
      INSERT INTO cached_organizations (org_id, parent_org_id, name, level, metadata, cached_at, updated_at)
      SELECT
        d.org_id,
        d.parent_org_id,
        d.name,
  ...
  def clear_cache do
    sql = "DELETE FROM cached_organizations"
