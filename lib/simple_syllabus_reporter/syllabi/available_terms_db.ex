defmodule SimpleSyllabusReporter.Syllabi.AvailableTermsDb do
  require Logger
  ...
  def upsert_terms(terms) when is_list(terms) do
    if Enum.empty?(terms) do
      ...
      sql = """
      INSERT INTO available_terms (term_id, term_name, status, cached_at, updated_at)
      SELECT
        d.term_id,
        d.term_name,
        'active',
  ...
  def count_active_terms do
    sql = "SELECT COUNT(*) as count FROM available_terms WHERE status = 'active'"
