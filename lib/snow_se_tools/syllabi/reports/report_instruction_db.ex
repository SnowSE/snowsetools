defmodule SnowSeTools.Reports.ReportInstructionDB do
  require Logger
  ...
  def list_for_element(required_element_id) do
    ...
    FROM syllabus_required_element_report_instructions
  ...
  def create(required_element_id, attrs) do
    case validate(attrs) do
      {:ok, d} ->
        sql = """
        INSERT INTO syllabus_required_element_report_instructions (required_element_id, content)
  ...
  def update(id, attrs) do
    case validate(attrs) do
      {:ok, d} ->
        sql = """
        UPDATE syllabus_required_element_report_instructions
        SET content    = $(content),
            updated_at = NOW()
        WHERE id = $(id)
        RETURNING id, required_element_id, content
