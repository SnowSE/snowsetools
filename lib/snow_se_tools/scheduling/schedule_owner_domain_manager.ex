defmodule SnowSeTools.Scheduling.ScheduleOwnerDomainManager do
  ...
  def request_terms(pid) when is_pid(pid) do
    GenServer.cast(__MODULE__, {:request_terms, pid})
  end

  def request_schedule_owners(pid, term_code) when is_pid(pid) and is_binary(term_code) do
    GenServer.cast(__MODULE__, {:request_schedule_owners, pid, term_code})
  end

  def request_schedule_owner_detail(pid, term_code, owner_key)
  ...
  def handle_cast({:request_terms, pid}, state) do
    ...
    send(pid, {:schedule_owner_terms, terms})
    {:noreply, state}
  end
  ...
  def handle_cast({:request_schedule_owners, pid, term_code}, state) do
    case Map.fetch(state.schedule_owners_by_term, term_code) do
      ...
      :error ->
        ...
        {:noreply, put_in(state.schedule_owners_by_term[term_code], schedule_owners)}
    end
  end
  ...
  def handle_cast({:request_schedule_owner_detail, pid, term_code, owner_key}, state) do
    ...
    case Enum.find(schedule_owners, &(&1.key == owner_key)) do
      nil ->
        send(pid, {:schedule_owner_detail, %{term_code: term_code, owner_key: owner_key, detail: nil}})

      detail ->
        send(pid, {:schedule_owner_detail, %{term_code: term_code, owner_key: owner_key, detail: detail}})
    end

    {:noreply, state}
  end

  def handle_cast({:snow_course_cache_updated, term_code}, state) do
    state = update_in(state.schedule_owners_by_term, &Map.delete(&1, term_code))
    schedule_owners = build_schedule_owners_for_term(term_code, state)
    state = put_in(state.schedule_owners_by_term[term_code], schedule_owners)

    ScheduleOwnerPubSub.broadcast_schedule_owners_changed(term_code)
    {:noreply, state}
  end

  def handle_cast({:snow_course_cache_deleted, term_code}, state) do
    state = update_in(state.schedule_owners_by_term, &Map.delete(&1, term_code))
    ScheduleOwnerPubSub.broadcast_schedule_owners_changed(term_code)
    {:noreply, state}
  end
  ...
  defp load_academic_programs do
    case ProgramDb.list_programs() do
      ...
      {:error, reason} ->
        Logger.error("ScheduleOwnerDomainManager failed to load academic programs: #{inspect(reason)}")
        []
    end
  end
  ...
  defp build_schedule_owners_for_term(term_code, state) do
    courses =
      case SnowCourseCacheDb.list_course_data_for_term(term_code: term_code) do
        {:ok, courses} -> courses
        {:error, reason} ->
          Logger.error("ScheduleOwnerDomainManager failed to load courses for term=#{term_code}: #{inspect(reason)}")
          []
      end
