defmodule SnowSeToolsWeb.Config.SyncStatusComponent do
  ...
  def update(%{sync_message: msg} = assigns, socket) do
    socket =
      socket
      # preserve other assigns, but sync_message is transient
  ...
  defp apply_sync_message(
         socket,
         {:sync_progress, %{total: total, completed: completed}}
       ) do
    percent =
      if total > 0,
        do: div(completed * 100, total),
        else: 0
  ...
  defp apply_sync_message(socket, {:sync_complete, _}) do
    socket
  ...
  def render(assigns) do
    ~H"""
    <div class="bg-slate-800/50 border border-slate-700 rounded-xl p-6" id={"#{@id}-sync-status"}>
